    ...
    for code in metar:
        if 'Z' in code:
            ...
            if not metarDict['windDir']:
                metarDict['windDir'].append(float('N'))
...
def formatText():
    ...
    print('Conditions at:           {} ({}) observed {}'.format(metarDict['station'],
                                                                stationLUT.loc[stationList.index(metarDict['station'])][
                                                                    0],
                                                                metarDict['date']))
...
entry = METAR(autoList, tk, listboxLength=7, width=300, matchesFunction=matches)
entry.grid(row=1)
